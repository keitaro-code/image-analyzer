...
画像から撮影地点を推測するためのデモアプリケーションです。ブラウザで画像をアップロードすると、バックエンドが元画像をそのまま Base64 化して OpenRouter のマルチモーダルモデル（現在は `qwen/qwen-2.5-vl-7b-instruct:free`）に渡し、人間が現地で観察するような推論プロセスで候補地点を返します。
...
- 画像内容とファイル名から組み立てた指示を添えて、人間の現地調査に近い観察→比較→結論の順番で OpenRouter の `qwen/qwen-2.5-vl-7b-instruct:free` モデルへ送信（マルチモーダルで画像認識が可能）
