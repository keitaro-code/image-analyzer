...
画像から撮影地点を推測するためのデモアプリケーションです。ブラウザで画像をアップロードすると、バックエンドが元画像をそのまま Base64 化して OpenRouter のマルチモーダルモデル（現在は `google/gemini-3-pro-preview`）に渡し、人間が現地で観察するような推論プロセスで候補地点を返します。
...
- 2026-02-14: マルチモーダルモデルを `qwen/qwen-2.5-vl-7b-instruct:free` に切り替え（画像認識強化・無料枠対応）。`backend/app.py` の `MODEL_NAME` を更新。
- 2026-02-15: マルチモーダルモデルを `google/gemini-3-pro-preview` に切り替え。`backend/app.py` の `MODEL_NAME` を更新。
- 2025-11-20: OpenRouter のデフォルトモデルを `google/gemini-3-pro-preview` に更新（`backend/app.py` の `MODEL_NAME`）。
...
- 画像内容とファイル名から組み立てた指示を添えて、人間の現地調査に近い観察→比較→結論の順番で OpenRouter の `google/gemini-3-pro-preview` モデルへ送信（マルチモーダルで画像認識が可能）
